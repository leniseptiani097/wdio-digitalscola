describe('test saucedemo', () => {
    it('test 1 - login successfully', async () => {
        ...
        // add item to cart
        const addItemButton1 = await $('button[data-test="add-to-cart-sauce-labs-backpack"]');
        await addItemButton1.waitForClickable({ timeout: 1000 });
        await addItemButton1.click();

        // add item to cart
        const addItemButton2 = await $('button[data-test="add-to-cart-sauce-labs-bike-light"]');
        await addItemButton2.waitForClickable({ timeout: 1000 });
        await addItemButton2.click();

        // add item to cart
        const addItemButton3 = await $('button[data-test="add-to-cart-sauce-labs-onesie"]');
        ...
        const btnRemove3 = await $('#remove-sauce-labs-onesie')
        ...
        const cartItem1 = await $('#item_4_title_link>div[data-test="inventory-item-name"]')
        await expect(cartItem1).toBeDisplayed();
        await expect(cartItem1).toHaveText('Sauce Labs Backpack');

        const cartItem2 = await $('#item_0_title_link>div[data-test="inventory-item-name"]')
        await expect(cartItem2).toBeDisplayed();
        await expect(cartItem2).toHaveText('Sauce Labs Bike Light');

        const cartItem3 = await $('#item_2_title_link>div[data-test="inventory-item-name"]')
        await expect(cartItem3).toBeDisplayed();
        await expect(cartItem3).toHaveText('Sauce Labs Onesie');
